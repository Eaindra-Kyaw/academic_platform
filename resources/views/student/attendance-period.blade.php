...
@section('welcome-text', 'Track your attendance across courses')
...
@section('content')
    <style>
        /* ============================================================
               DESIGN SYSTEM (matches lecturer page)
               ============================================================ */
        :root {
            --primary: #0A2463;
    ...
    <div class="period-controls">
        <div class="period-tabs">
            {{-- 1. This Week --}}
            <a href="{{ route('student.attendance.period', ['period' => 'weekly', 'offset' => 0]) }}"
                class="btn-tab {{ $period == 'weekly' ? 'active' : '' }}">
                <i class="bi bi-calendar-week"></i> This Week
            </a>

            {{-- 2. This Month --}}
            <a href="{{ route('student.attendance.period', ['period' => 'monthly', 'offset' => 0]) }}"
            ...
            {{-- 4. Semester (overall) --}}
            <a href="{{ route('student.attendance.period', ['period' => 'overall']) }}"
        ...
        <div class="period-nav">
            @if ($period != 'overall' && $period != 'custom')
                <a href="{{ route('student.attendance.period', ['period' => $period, 'offset' => $offset - 1]) }}"
                    class="btn-nav">
        ...
        <div class="course-filter">
            <form method="GET" id="courseFilterForm" style="display:flex; align-items:center; gap:4px;">
                ...
                @if (!empty($courseId))
                    <a href="{{ route('student.attendance.period', ['period' => $period, 'offset' => $offset]) }}"
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div class="sub">{{ $totalStudents > 0 ? round(($eligibleCount / $totalStudents) * 100) : 0 }}% of courses
            </div>
        </div>
        <div class="stat-card">
            <div class="number yellow">{{ $warningCount }}</div>
            <div class="label"> Warning</div>
            <div class="sub">{{ $totalStudents > 0 ? round(($warningCount / $totalStudents) * 100) : 0 }}% of courses
            </div>
        </div>
        <div class="stat-card">
            <div class="number red">{{ $atRiskCount }}</div>
            <div class="label"> At Risk</div>
            <div class="sub">{{ $totalStudents > 0 ? round(($atRiskCount / $totalStudents) * 100) : 0 }}% of courses
            </div>
        </div>
        <div class="stat-card">
    ...
    @forelse ($courses as $course)
        @php
            ...
            $total = $students->count(); // should be 1 for a student
            $eligible = $students->where('status', 'Eligible')->count();
            $atRisk = $students->where('risk_level', 'High')->count();
            $avgAtt = $total > 0 ? round($students->avg('attendance_percentage'), 1) : 0;
        ...
        <div class="course-card">
            <div class="course-header">
                <div class="title">
                    ...
                    <span class="count">{{ $total }} student</span>
                    @if ($total > 0)
                        ...
                        <tbody>
                            @foreach ($students as $index => $student)
                                @php
                                    ...
                                    $hasEval = ($student->total_courses ?? 0) > 0 || $att > 0;
    ...
    @empty
        <div class="empty-state" style="padding:48px;">
            ...
            <h5 style="color:var(--gray-700);margin-bottom:4px;">No Courses Enrolled</h5>
            <p>You are not enrolled in any active courses at the moment.</p>
        </div>
    @endforelse
    ...
    @push('scripts')
        <script>
            ...
            // Table sorting (same as lecturer)
            document.addEventListener('DOMContentLoaded', function() {
                        ...
                        header.addEventListener('click', function() {
                            ...
                            rows.sort(function(a, b) {
                                ...
                                if (dataType === 'number') {
                                    ...
                                    return order === 'asc' ? aVal.localeCompare(bVal) :
