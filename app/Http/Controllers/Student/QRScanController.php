...
class QRScanController extends Controller
{
    ...
    // Semester QR Scan (for Static QR - same QR all semester)
   public function semesterScan(Request $request)
{
    ...
    \Log::info('Semester scan - Student: ' . $student->id . ', Token: ' . $token . ', Course: ' . $courseId);
    ...
    // Get the active session for this course (the one the lecturer is viewing)
    ...
    if (!$session) {
        return view('student.scan-result', [
            'success' => false,
            'message' => 'No active session for this course. Please ask your lecturer to start a session.'
        ]);
    }
    ...
    if ($existingRecord) {
        return view('student.scan-result', [
            'success' => false,
            'message' => 'Attendance already recorded for this session. Status: ' . $existingRecord->status
        ]);
    }
    ...
    if ($minutesLate <= 10) {
        $status = 'present';
        $userMessage = 'Attendance marked as PRESENT! You arrived on time.';
    ...
    // Dynamic Session QR Scan (for traditional QR that changes each session)
    public function processScan(Request $request)
{
    ...
    $student = Auth::user();
    \Log::info('Student logged in: ' . ($student ? $student->id . ' - ' . $student->name : 'NO STUDENT LOGGED IN'));
    ...
    \Log::info('Attendance recorded successfully - Status: ' . $status);
